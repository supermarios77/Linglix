...
export function MarqueeTicker({ items }: MarqueeTickerProps) {
  return (
    <div 
      className="w-full bg-[#111] text-white py-4 overflow-hidden whitespace-nowrap relative -rotate-1 scale-[1.02] -mt-10 z-[5] border-t border-b border-[#333]"
    ...
    >
      <div className="inline-block animate-[marquee_20s_linear_infinite]">
        ...
        {items.map((item, index) => (
          <span key={`duplicate2-${index}`} className="text-lg font-medium uppercase px-10 tracking-wider inline-block">
            {item}
          </span>
        ))}
      </div>
