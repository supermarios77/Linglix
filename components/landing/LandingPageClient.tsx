...
export function LandingPageClient({ session, locale }: LandingPageClientProps) {
  ...
  return (
    <>
      {/* Header */}
      <header className="fixed top-0 z-[1000] w-full h-20 flex justify-between items-center px-4 md:px-12 bg-[rgba(250,250,250,0.85)] dark:bg-[rgba(17,17,17,0.85)] backdrop-blur-xl border-b border-[rgba(0,0,0,0.03)] dark:border-[rgba(255,255,255,0.05)]">
        {/* Mobile Menu Button */}
        <button
        ...
        <Link href={`/${locale}`} className="font-bold text-xl md:text-2xl tracking-[-0.03em] absolute left-1/2 -translate-x-1/2 md:relative md:left-auto md:translate-x-0 text-black dark:text-white">
          Linglix<span className="text-[#111] dark:text-white">.</span>
        </Link>
        ...
        <div className="flex gap-3 md:gap-5 items-center">
          ...
          {session ? (
            <Link href={`/${locale}/dashboard`}>
              <button className="px-4 py-2 rounded-full text-sm font-medium bg-[#111] dark:bg-white text-white dark:text-black transition-all hover:scale-105">
                {tCommon("dashboard")}
              </button>
            </Link>
          ) : (
            <>
              <Link href={`/${locale}/auth/signin`}>
                <button className="hidden md:block px-4 py-2 rounded-full text-sm font-medium text-[#444] dark:text-[#aaa] hover:text-black dark:hover:text-white transition-colors">
                  {tAuth("signInTitle")}
                </button>
              </Link>
        ...
        {mobileMenuOpen && (
          <div className="absolute top-20 left-0 right-0 bg-[rgba(250,250,250,0.98)] dark:bg-[rgba(17,17,17,0.98)] backdrop-blur-xl border-b border-[rgba(0,0,0,0.03)] dark:border-[rgba(255,255,255,0.05)] md:hidden">
      ...
      <button
        onClick={scrollToTop}
        className={`fixed bottom-8 right-8 z-[999] w-12 h-12 rounded-full bg-white/80 dark:bg-black/80 backdrop-blur-xl border border-[rgba(0,0,0,0.06)] dark:border-[rgba(255,255,255,0.1)] shadow-[0_8px_30px_rgba(0,0,0,0.08)] flex items-center justify-center transition-all duration-300 hover:scale-110 hover:shadow-[0_12px_40px_rgba(0,0,0,0.12)] active:scale-95 ${
          showScrollTop ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4 pointer-events-none"
        }`}
        aria-label="Scroll to top"
      >
        <ArrowUp className="w-5 h-5 text-[#111] dark:text-white" />
      </button>
    </>
  );
}
